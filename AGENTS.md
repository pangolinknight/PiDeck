...
- 公共控件优先沉淀到 `src/renderer/src/components/ui/`，例如 `SelectField`。当两个以上页面需要相同按钮、输入、下拉、badge、列表行或弹层结构时，应优先抽为共享组件或共享 class。
...
- `PiDeckDepartureMono` 可用于短数字、徽标、度量值或品牌化技术标识；不要大面积用于正文，避免降低可读性。
